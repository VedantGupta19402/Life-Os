...
export function MetricCards() {
  return (
    <section className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" aria-label="Today's metrics">
      {metrics.map(({ label, value, unit, status, statusTone, icon: Icon, iconTone }) => (
        <motion.article whileHover={{ y: -5, boxShadow: "0 12px 24px rgba(20, 62, 51, .08)" }} transition={{ duration: 0.22 }} className="flex min-h-[158px] gap-4 rounded-[10px] border border-[#e6ded2] bg-[#fcfaf6] px-4 py-7" key={label}>
          ...
          <div>
            ...
            <div className="font-serif text-[37px] leading-none tracking-[-1.7px]">{value}<span className="ml-1 font-sans text-[16px] tracking-normal">{unit}</span></div>
            <p className="mt-5 whitespace-nowrap text-[13px] text-[#3e4541]"><i className={`mr-2 inline-block h-2 w-2 rounded-full ${statusTone === "warm" ? "bg-[#c45f37]" : "bg-[#1d8963]"}`} />{status}</p>
          </div>
        </motion.article>
      ))}
    </section>
